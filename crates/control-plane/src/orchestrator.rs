use lsdc_common::crypto::{
    ContractAmendmentProposal, PriceDecision, ProofBundle, SanctionProposal, Sha256Hash,
};
use lsdc_common::dsp::ContractAgreement;
use lsdc_common::error::{LsdcError, Result};
use lsdc_common::liquid::CsvTransformManifest;
use lsdc_common::traits::{
    DataPlane, EnclaveJobRequest, EnclaveManager, EnforcementHandle, PricingOracle,
    TrainingMetrics,
};
use std::sync::Arc;
...
pub struct BatchLineageResult {
    ...
    pub amendment_proposal: Option<ContractAmendmentProposal>,
...
impl Orchestrator {
    ...
    pub async fn request_price_decision(
        ...
        let shapley_value = pricing_oracle
            .evaluate_utility(dataset_id, transformed_asset_hash, metrics)
            .await?;
        pricing_oracle
    ...
    pub async fn run_batch_csv_lineage(
        ...
        let forgetting_valid = verify_proof_of_forgetting(&job_result.proof_bundle.proof_of_forgetting)?;
        ...
        let amendment_proposal = forgetting_valid.then(|| ContractAmendmentProposal {
            agreement_id: request.agreement.agreement_id.0.clone(),
            proposed_price: price_decision.adjusted_price,
            approval_required: price_decision.approval_required,
            price_decision_signature: price_decision.signature_hex.clone(),
        });
