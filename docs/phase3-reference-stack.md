...
This is still a truthful prototype. The stack demonstrates guarded transfer, protected CSV transforms, provenance receipts, proof-of-forgetting, and advisory pricing without claiming recursive zk rollups, live enclave orchestration, or autonomous contract mutation.
...
- Default multi-hop sovereignty is demonstrated with `DevReceiptProofEngine`, not recursive zk rollups.
- `RISC Zero` is feature-gated and only exercised for single-hop lineage through `control-plane-api`.
